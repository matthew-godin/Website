const SKILLS = {
    title: 'Skills',
    programmingLanguages: {
        title: 'Programming Languages',
        content: [
            { name: 'TypeScript', desktop: 3, mobile: 8 },
            { name: 'JavaScript', desktop: 3, mobile: 8 },
            { name: 'Java', desktop: 2, mobile: 4 },
            { name: 'Python', desktop: 2, mobile: 4 },
            { name: 'Ruby', desktop: 2, mobile: 6 },
            { name: 'C#', desktop: 1, mobile: 6 },
            { name: 'Go', desktop: 1, mobile: 6 },
            { name: 'SQL', desktop: 2, mobile: 6 },
            { name: 'C++', desktop: 1, mobile: 5 },
            { name: 'C', desktop: 1, mobile: 4 },
            { name: 'PHP', desktop: 2, mobile: 5 },
            { name: 'Kotlin', desktop: 2, mobile: 5 },
            { name: 'Swift', desktop: 2, mobile: 5 },
        ],
        mobileNumColumns: [
            4,
            4,
            5
        ]
    },
    tools: {
        title: 'Tools',
        content: [
            { name: 'Git', desktop: 2, mobile: 8 },
            { name: 'Bash', desktop: 2, mobile: 8 },
            { name: 'PowerShell', desktop: 3, mobile: 8 },
            { name: 'Docker', desktop: 2, mobile: 9 },
            { name: 'VS Code', desktop: 3, mobile: 6 },
            { name: 'Visual Studio', desktop: 3, mobile: 9 },
            { name: 'IntelliJ', desktop: 3, mobile: 6 },
            { name: 'Android Studio', desktop: 4, mobile: 12 },
            { name: 'Xcode', desktop: 2, mobile: 6 }
        ],
        mobileNumColumns: [
            3,
            3,
            3
        ]
    },
    frameworks: {
        title: 'Frameworks',
        content: [
            { name: 'Node.js', desktop: 2, mobile: 5 },
            { name: 'React', desktop: 2, mobile: 5 },
            { name: 'ASP.NET', desktop: 2, mobile: 5 },
            { name: 'Ruby on Rails', desktop: 3, mobile: 9 },
            { name: 'Spring', desktop: 1, mobile: 6 },
            { name: 'Laravel', desktop: 2, mobile: 6 },
            { name: 'Vue', desktop: 1, mobile: 6 },
            { name: 'PyTorch', desktop: 2, mobile: 6 },
            { name: 'Kafka', desktop: 2, mobile: 6 },
            { name: 'Spark', desktop: 1, mobile: 5 },
            { name: 'Deno', desktop: 2, mobile: 5 },
            { name: 'Jest', desktop: 1, mobile: 4 },
            { name: 'NUnit', desktop: 2, mobile: 5 },
            { name: 'JUnit', desktop: 1, mobile: 5 }
        ],
        mobileNumColumns: [
            4,
            5,
            5
        ]
    },
    nextId: 'experience'
};

export default SKILLS;
